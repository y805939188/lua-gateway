...
function Counter:inc(value, label_values)
  self.prometheus:inc(self.name, label_values, value or 1)
end
...
function Gauge:set(value, label_values)
  ...
  self.prometheus:set(self.name, label_values, value)
end
...
function Histogram:observe(value, label_values)
  ...
  self.prometheus:histogram_observe(self.name, label_values, value)
end
...
function Prometheus:counter(name, description)
  ...
  return Counter:new{name=name, prometheus=self}
end
...
-- Returns:
--   a Gauge object.
function Prometheus:gauge(name, description)
  if not self.initialized then
    ngx.log(ngx.ERR, "Prometheus module has not been initialized")
    return
  end
  ...
  return Gauge:new{name=name, prometheus=self}
end
...
function Prometheus:histogram(name, description, buckets)
  ...
  return Histogram:new{name=name, prometheus=self}
end
...
function Prometheus:inc(name, label_values, value)
...
function Prometheus:set(name, label_values, value)
  local key = name .. "{" .. label_values .. "}"
  self:set_key(key, value)
end
...
function Prometheus:histogram_observe(name, label_values, value)
  local name_bucket = name .. "_bucket"
  local prefix = ""
  if label_values then
    prefix = label_values .. ',le="'
  else
    prefix = 'le="'
  end
  self:inc(name .. "_count", label_values, 1)
  self:inc(name .. "_sum", label_values, value)
  self:inc(name_bucket, prefix .. 'Inf' .. '"', 1)
  for _, bucket in ipairs(self.buckets[name]) do
    if value <= bucket then
      self:inc(name_bucket, prefix .. self.bucket_format[name]:format(bucket) .. '"', 1)
    end
  end
end
...
function Prometheus:construct_labels(label_names, label_values)
  if label_names == nil and label_values == nil then
    return ""
  elseif label_names == nil and label_values ~= nil then
    self:log_error("Expected no labels, got ",  #label_values)
    return ""
  elseif label_values == nil and label_names ~= nil then
    self:log_error("Expected ", #label_names, " labels, got none")
    return ""
  elseif #label_names ~= #label_values * 2 then
    self:log_error("Wrong number of labels. Expected ",
           #label_names / 2, ", got ", #label_values)
    return ""
  else
    for i, k in ipairs(label_values) do
      if label_values[i] == nil then
        self:log_error("Unexpected nil label")
        return ""
      end
    end
  end
  local l_names = copy_table(label_names)
  for idx, key in ipairs(label_values) do
    l_names[idx*2] = label_values[idx] .. '"'
  end
  return table.concat(l_names, '')
end

function Prometheus:cached_label_names(label_names)
    if label_names == nil then
        return {}
    end
    local label_parts = {}
    local count = 1
